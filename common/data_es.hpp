...
namespace XuChat
{
    ...
    public:
        ...
        bool createIndex()
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户信息索引创建失败!");
                return false;
            }
            log_info(logger, "用户信息索引创建成功!");
            return true;
        }
                        ...
                        const std::string &avatar_id)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户数据插入/更新失败!");
                return false;
            }
            log_info(logger, "用户数据新增/更新成功!");
            return true;
        }
        std::vector<XuChat::User> search(const std::string &key, const std::vector<std::string> &uid_list)
        {
            ...
            {
                log_error(logger, "用户搜索结果为空，或者结果不是数组类型");
                return res;
            }
            int sz = json_user.size();
            debug(logger, "检索结果条目数量：%d", sz);
            for (int i = 0; i < sz; i++)
            {
                XuChat::User user;
    ...
    public:
        ...
        bool createIndex()
        {
            ...
            if (ret == false)
            {
                log_error(logger, "消息信息索引创建失败!");
                return false;
            }
            log_info(logger, "消息信息索引创建成功!");
            return true;
        }
                        ...
                        const std::string &content)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "消息数据插入/更新失败!");
                return false;
            }
            log_info(logger, "消息数据新增/更新成功!");
            return true;
        }
        bool remove(const std::string &mid)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "消息数据删除失败!");
                return false;
            }
            log_info(logger, "消息数据删除成功!");
            return true;
        }
        std::vector<XuChat::Message> search(const std::string &key, const std::string &ssid)
        {
