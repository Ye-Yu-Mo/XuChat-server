...
#define SPEECH_RECOGNITION "/service/speech/recognition"
    ...
    private:
        ...
        {
            debug(logger, "websocket长连接建立成功 %d", (size_t)_ws_server.get_con_from_hdl(hdl).get());
        }
        void onClose(websocketpp::connection_hdl hdl)
        {
            ...
            if (ret == false)
            {
                warn(logger, "长连接断开，未找到长连接对应的客户端信息！");
                return;
            }
            // 1. 移除登录会话信息
        ...
        void onMessage(websocketpp::connection_hdl hdl, server_t::message_ptr msg)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "长连接身份识别失败：正文反序列化失败！");
            ...
            if (!uid)
            {
                log_error(logger, "长连接身份识别失败：未找到会话信息 %s！", ssid.c_str());
                _ws_server.close(hdl, websocketpp::close::status::unsupported_data, "未找到会话信息!");
                return;
            }
            // 5. 会话信息存在，则添加长连接管理
            _connections->insert(conn, *uid, ssid);
            debug(logger, "新增长连接管理：%s-%s-%d", ssid.c_str(), uid->c_str(), (size_t)conn.get());
            keepAlive(conn);
        }
        void GetPhoneVerifyCode(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "获取短信验证码请求正文反序列化失败！");
            ...
            if (!channel)
            {
                log_error(logger, "%s 未找到可提供业务处理的用户子服务节点！", req.request_id().c_str());
            ...
            if (cntl.Failed())
            {
                log_error(logger, "%s 用户子服务调用失败！", req.request_id().c_str());
        ...
        void UserRegister(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户名注册请求正文反序列化失败！");
        ...
        void UserLogin(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户登录请求正文反序列化失败！");
        ...
        void PhoneRegister(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "手机号注册请求正文反序列化失败！");
        ...
        void PhoneLogin(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "手机号登录请求正文反序列化失败！");
        ...
        void GetUserInfo(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "获取用户信息请求正文反序列化失败！");
            ...
            if (!uid)
            {
                log_error(logger, "%s 获取登录会话关联用户信息失败！", ssid);
            ...
            if (!channel)
            {
                log_error(logger, "%s 未找到可提供业务处理的用户子服务节点！", req.request_id());
                return err_response("未找到可提供业务处理的用户子服务节点！");
            }
            XuChat::UserService_Stub stub(channel.get());
            brpc::Controller cntl;
            ...
            if (cntl.Failed())
            {
                log_error(logger, "%s 用户子服务调用失败！", req.request_id());
                return err_response("用户子服务调用失败！");
            }
            // 3. 得到用户子服务的响应后，将响应内容进行序列化作为http响应正文
            response.set_content(rsp.SerializeAsString(), "application/x-protbuf");
        ...
        void SetUserAvatar(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户头像设置请求正文反序列化失败！");
            ...
            if (!uid)
            {
                log_error(logger, "%s 获取登录会话关联用户信息失败！", ssid.c_str());
                return err_response("获取登录会话关联用户信息失败！");
            }
            req.set_user_id(*uid);
            // 2. 将请求转发给用户子服务进行业务处理
        ...
        void SetUserNickname(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户昵称设置请求正文反序列化失败！");
        ...
        void SetUserDescription(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户签名设置请求正文反序列化失败！");
        ...
        void SetUserPhoneNumber(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户手机号设置请求正文反序列化失败！");
        ...
        void GetFriendList(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "获取好友列表请求正文反序列化失败！");
            ...
            if (cntl.Failed())
            {
                log_error(logger, "%s 好友子服务调用失败！", req.request_id().c_str());
        ...
        void FriendAdd(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "申请好友请求正文反序列化失败！");
            ...
            if (rsp.success() && conn)
            {
                debug(logger, "找到被申请人 %s 长连接，对其进行好友申请通知", req.respondent_id().c_str());
                auto user_rsp = _GetUserInfo(req.request_id(), *uid);
                if (!user_rsp)
                {
                    log_error(logger, "%s 获取当前客户端用户信息失败！", req.request_id().c_str());
        ...
        void FriendAddProcess(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "好友申请处理请求正文反序列化失败！");
            ...
            if (rsp.success())
            {
                ...
                if (!process_user_rsp)
                {
                    log_error(logger, "%s 获取用户信息失败！", req.request_id().c_str());
                ...
                if (process_conn)
                    debug(logger, "找到处理人的长连接！");
                else
                    debug(logger, "未找到处理人的长连接！");
                auto apply_conn = _connections->connection(req.apply_user_id());
                if (apply_conn)
                    debug(logger, "找到申请人的长连接！");
                else
                    debug(logger, "未找到申请人的长连接！");
                // 4. 将处理结果给申请人进行通知
                if (apply_conn)
                {
                    NotifyMessage notify;
        ...
        void FriendRemove(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "删除好友请求正文反序列化失败！");
            ...
            if (rsp.success() && conn)
            {
                log_error(logger, "对被删除人 %s 进行好友删除通知！", req.peer_id());
                NotifyMessage notify;
        ...
        void FriendSearch(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "用户搜索请求正文反序列化失败！");
        ...
        void GetPendingFriendEventList(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "获取待处理好友申请请求正文反序列化失败！");
        ...
        void GetChatSessionList(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "获取聊天会话列表请求正文反序列化失败！");
            ...
            if (cntl.Failed())
            {
                log_error(logger, "%s 好友子服务调用失败！", req.request_id());
                return err_response("好友子服务调用失败！");
            }
            // 5. 向客户端进行响应
            response.set_content(rsp.SerializeAsString(), "application/x-protbuf");
        ...
        void GetChatSessionMember(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "获取聊天会话成员请求正文反序列化失败！");
        ...
        void ChatSessionCreate(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "创建聊天会话请求正文反序列化失败！");
            ...
            if (rsp.success())
            {
                for (int i = 0; i < req.member_id_list_size(); i++)
                {
                    ...
                    if (!conn)
                    {
                        debug(logger, "未找到群聊成员 %s 长连接", req.member_id_list(i).c_str());
                        continue;
                    }
                    NotifyMessage notify;
                    notify.set_notify_type(NotifyType::CHAT_SESSION_CREATE_NOTIFY);
                    ...
                    conn->send(notify.SerializeAsString(), websocketpp::frame::opcode::value::binary);
                    debug(logger, "对群聊成员 %s 进行会话创建通知", req.member_id_list(i));
                }
            }
            // 5. 向客户端进行响应
        ...
        void GetHistoryMsg(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "获取区间消息请求正文反序列化失败！");
            ...
            if (cntl.Failed())
            {
                log_error(logger, "%s 消息存储子服务调用失败！", req.request_id().c_str());
        ...
        void GetRecentMsg(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "获取最近消息请求正文反序列化失败！");
            ...
            if (cntl.Failed())
            {
                log_error(logger, "%s 消息存储子服务调用失败！", req.request_id());
                return err_response("消息存储子服务调用失败！");
            }
            // 5. 向客户端进行响应
            response.set_content(rsp.SerializeAsString(), "application/x-protbuf");
        ...
        void MsgSearch(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "消息搜索请求正文反序列化失败！");
        ...
        void GetSingleFile(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "单文件下载请求正文反序列化失败！");
            ...
            if (cntl.Failed())
            {
                log_error(logger, "%s 文件存储子服务调用失败！", req.request_id().c_str());
        ...
        void PutSingleFile(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "单文件上传请求正文反序列化失败！");
        ...
        void PutMultiFile(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "批量文件上传请求正文反序列化失败！");
        ...
        void SpeechRecognition(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "语音识别请求正文反序列化失败！");
            ...
            XuChat::SpeechService_Stub stub(channel.get());
            brpc::Controller cntl;
            stub.SpeechRecognition(&cntl, &req, &rsp, nullptr);
            if (cntl.Failed())
            {
                log_error(logger, "%s 语音识别子服务调用失败！", req.request_id().c_str());
        ...
        void NewMessage(const httplib::Request &request, httplib::Response &response)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "新消息请求正文反序列化失败！");
            ...
            if (cntl.Failed())
            {
                log_error(logger, "%s 消息转发子服务调用失败！", req.request_id().c_str());
    ...
    public:
        ...
        GatewayServer::ptr build()
        {
            if (!_redis_client)
            {
                log_error(logger, "还未初始化Redis客户端模块！");
                abort();
            }
