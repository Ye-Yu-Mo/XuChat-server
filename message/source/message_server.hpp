...
namespace XuChat
{
    class MessageServiceImpl : public XuChat::MsgStorageService
    {
    public:
        MessageServiceImpl(
                                   ...
                                   ::google::protobuf::Closure *done)
        {
            ...
            for (const auto &msg : msg_lists)
            {
                if (msg.file_id().empty())
                    continue;
                debug(logger, "需要下载的文件ID： %s", msg.file_id().c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s 批量文件数据下载失败！", rid.c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s 批量用户数据获取失败！", rid.c_str());
        ...
        void onMessage(const char *body, size_t sz)
        {
            ...
            case MessageType::STRING:
                ...
                if (ret == false)
                {
                    log_error(logger, "文本消息向存储引擎进行存储失败！");
                    return;
                }
                break;
            ...
            XuChat::Message msg(message.message_id(),
                                 message.chat_session_id(),
                                 message.sender().user_id(),
    ...
    private:
                      ...
                      std::unordered_map<std::string, UserInfo> &user_lists)
        {
            ...
            if (!channel)
            {
                log_error(logger, "%s 没有可供访问的用户子服务节点！", _user_service_name.c_str());
                return false;
            }
            ...
            {
                log_error(logger, "用户子服务调用失败：%s！", cntl.ErrorText().c_str());
                return false;
            }
                      ...
                      std::unordered_map<std::string, std::string> &file_data_lists)
        {
            ...
            if (!channel)
            {
                log_error(logger, "%s 没有可供访问的文件子服务节点！", _file_service_name.c_str());
                return false;
            }
            ...
            if (cntl.Failed() == true || rsp.success() == false)
            {
                log_error(logger, "文件子服务调用失败：%s！", cntl.ErrorText().c_str());
                return false;
            }
