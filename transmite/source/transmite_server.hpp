...
namespace XuChat
{
    class TransmiteServiceImpl : public XuChat::MsgTransmitService
    {
    public:
                               ...
                               ::google::protobuf::Closure *done) override
        {
            ...
            if (!channel)
            {
                log_error(logger, "%s-%s 没有可供访问的用户子服务节点！", rid.c_str(), _user_service_name.c_str());
            ...
            {
                log_error(logger, "%s - 用户子服务调用失败：%s！", request->request_id().c_str(), cntl.ErrorText().c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 持久化消息发布失败：%s！", request->request_id().c_str(), cntl.ErrorText().c_str());
    ...
    public:
        ...
        void make_rpc_server(uint16_t port, int32_t timeout, uint8_t num_threads)
        {
            if (!_mq_client)
            {
                log_error(logger, "还未初始化消息队列客户端模块！");
                abort();
            }
            if (!_mm_channels)
            {
