...
namespace XuChat
{
    class UserServiceImpl : public XuChat::UserService
    {
    public:
        ...
        bool password_check(const std::string &password)
        {
            ...
            {
                log_error(logger, "密码长度不合法：%s-%d", password.c_str(), password.size());
                return false;
            }
            for (int i = 0; i < password.size(); i++)
            {
                ...
                {
                    log_error(logger, "密码字符不合法：%s", password.c_str());
                    return false;
                }
            }
            return true;
                                  ...
                                  ::google::protobuf::Closure *done)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 用户名长度不合法！", request->request_id().c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 密码格式不合法！", request->request_id().c_str());
            ...
            if (user)
            {
                log_error(logger, "%s - 用户名被占用- %s！", request->request_id().c_str(), nickname.c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - Mysql数据库新增数据失败！", request->request_id().c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - ES搜索引擎新增数据失败！", request->request_id().c_str());
                               ...
                               ::google::protobuf::Closure *done)
        {
            ...
            {
                log_error(logger, "%s - 用户名或密码错误 - %s-%s！", request->request_id().c_str(), nickname.c_str(), password.c_str());
            ...
            if (ret == true)
            {
                log_error(logger, "%s - 用户已在其他地方登录 - %s！", request->request_id().c_str(), nickname.c_str());
                                        ...
                                        ::google::protobuf::Closure *done)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 手机号码格式错误 - %s！", request->request_id().c_str(), phone.c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 短信验证码发送失败 - %s！", request->request_id(), phone.c_str());
                                   ...
                                   ::google::protobuf::Closure *done)
        {
            ...
            if (vcode != code)
            {
                log_error(logger, "%s - 验证码错误 - %s-%s！", request->request_id().c_str(), code_id.c_str(), code.c_str());
            ...
            if (user)
            {
                log_error(logger, "%s - 该手机号已注册过用户 - %s！", request->request_id().c_str(), phone.c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 向数据库添加用户信息失败 - %s！", request->request_id().c_str(), phone.c_str());
                                ...
                                ::google::protobuf::Closure *done)
        {
            ...
            if (!user)
            {
                log_error(logger, "%s - 该手机号未注册用户 - %s！", request->request_id().c_str(), phone.c_str());
            ...
            if (ret == true)
            {
                log_error(logger, "%s - 用户已在其他地方登录 - %s！", request->request_id().c_str(), phone.c_str());
                return err_response(request->request_id(), "用户已在其他地方登录!");
            }
            // 4. 构造会话 ID，生成会话键值对，向 redis 中添加会话信息以及登录标记信息
            std::string ssid = uuid();
                                 ...
                                 ::google::protobuf::Closure *done)
        {
            ...
            if (!user)
            {
                log_error(logger, "%s - 未找到用户信息 - %s！", request->request_id().c_str(), uid.c_str());
            ...
            if (!user->avatar_id().empty())
            {
                ...
                if (!channel)
                {
                    log_error(logger, "%s - 未找到文件管理子服务节点 - %s - %s！",
                              request->request_id(), _file_service_name.c_str(), uid.c_str());
                ...
                {
                    log_error(logger, "%s - 文件子服务调用失败：%s！", request->request_id().c_str(), cntl.ErrorText().c_str());
                                      ...
                                      ::google::protobuf::Closure *done)
        {
            ...
            {
                log_error(logger, "%s - 从数据库查找的用户信息数量不一致 %d-%d！",
                          request->request_id().c_str(), request->users_id_size(), users.size());
            ...
            if (!channel)
            {
                log_error(logger, "%s - 未找到文件管理子服务节点 - %s！", request->request_id().c_str(), _file_service_name.c_str());
                return err_response(request->request_id(), "未找到文件管理子服务节点!");
            }
            XuChat::FileService_Stub stub(channel.get());
            ...
            if (cntl.Failed() == true || rsp.success() == false)
            {
                log_error(logger, "%s - 文件子服务调用失败：%s - %s！", request->request_id(),
                                   ...
                                   ::google::protobuf::Closure *done)
        {
            ...
            if (!channel)
            {
                log_error(logger, "%s - 未找到文件管理子服务节点 - %s！", request->request_id(), _file_service_name);
                return err_response(request->request_id(), "未找到文件管理子服务节点!");
            }
            XuChat::FileService_Stub stub(channel.get());
            XuChat::PutSingleFileReq req;
            XuChat::PutSingleFileRsp rsp;
            req.set_request_id(request->request_id());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 更新数据库用户头像ID失败 ：%s！", request->request_id().c_str(), avatar_id.c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 更新搜索引擎用户头像ID失败 ：%s！", request->request_id().c_str(), avatar_id.c_str());
                                     ...
                                     ::google::protobuf::Closure *done)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 更新数据库用户昵称失败 ：%s！", request->request_id().c_str(), new_nickname.c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 更新搜索引擎用户昵称失败 ：%s！", request->request_id().c_str(), new_nickname.c_str());
                                        ...
                                        ::google::protobuf::Closure *done)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 更新数据库用户签名失败 ：%s！", request->request_id().c_str(), new_description.c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 更新搜索引擎用户签名失败 ：%s！", request->request_id().c_str(), new_description.c_str());
                                        ...
                                        ::google::protobuf::Closure *done)
        {
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 更新数据库用户手机号失败 ：%s！", request->request_id().c_str(), new_phone.c_str());
            ...
            if (ret == false)
            {
                log_error(logger, "%s - 更新搜索引擎用户手机号失败 ：%s！", request->request_id().c_str(), new_phone.c_str());
    ...
    public:
                                   ...
                                   const std::string &file_service_name)
        {
            ...
            debug(logger, "设置文件子服务为需添加管理的子服务：%s", file_service_name.c_str());
        ...
        void make_rpc_server(uint16_t port, int32_t timeout, uint8_t num_threads)
        {
            ...
            if (!_redis_client)
            {
                log_error(logger, "还未初始化Redis数据库模块！");
                abort();
            }
            if (!_mm_channels)
            {
